...
function getArtistByIndex(array, number) {
  const string =`the artist at index ${array[number].id} is ${array[number].name}`;
  return string;
}
...
function removeArtist(array, number) {
  const anotherCopy = [...array];
  anotherCopy.splice(number, 1);
  return anotherCopy;
}
...
function addArtist(array, name, years, genre, nationality, bio) {
  const anotherNotherCopy = [...array];
  const obj = {
    name: name, years: years + " - 2023", genre: genre, nationality: nationality, bio: bio
  }
  anotherNotherCopy.push(obj);
  return anotherNotherCopy;
}
addArtist(artists, "Ben Johnson", "1987", "Web Design", "American", "lorem ipsum");
...
function artistByCountry(array, nationality){
  const artistCountry = [];
  for(let i = 0; i < array.length; i++) { if(array[i].nationality.includes(nationality)) {
      artistCountry.push(array[i]);
